...
import dev.mayutama.smartbook.common.util.TypeConverter;
...
public class RestEngine {
    ...
    public MappingJacksonValue applyJsonFilter(Object object, RestParamRequest paramRequest)
    {
        return applyJsonFilter(object, object.getClass(), paramRequest);
    }

    public MappingJacksonValue applyJsonFilter(Object object, Class<?> objectClass, RestParamRequest paramRequest)
    {
        return applyJsonFilter(object, objectClass, paramRequest.getExpand(), paramRequest.getInclude(), paramRequest.getExclude());
    }
    ...
    public Set<RestFilter> processFilter(Class<?> clazz, Map<String, String[]> rawFilter) throws ParseException, ReflectiveOperationException {
        ...
        for(Map.Entry<String, String[]> entry : rawFilter.entrySet())
        {
            for(String rawValue : entry.getValue())
            {
                ...
                {
                    filter.setOperator(RestFilter.Operator.valueOf(operatorMatcher.group(1)));
                    rawValue = operatorMatcher.group(2);
                }
                else
                    filter.setOperator(RestFilter.Operator.EQ);
